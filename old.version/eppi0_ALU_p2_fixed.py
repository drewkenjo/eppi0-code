...
def get_sigs(grbsa, tshift=0):
    ...
    for kk in grbsa:
        ...
        alt,att = [(alt,att) for q2,xb,tt,alt,att in atts if abs(q2-qmean)<0.051 and abs(xb-xmean)<0.051 and abs(tt-tmean)<0.051][0]

        f1=ROOT.TF1("f1","[0]*sin(x*TMath::DegToRad())/(1+[1]*cos(2*x*TMath::DegToRad()))+[2]",0,360)
        f1.SetParameters(0.1,att)
        f1.FixParameter(1,att)
        grbsa[kk].Fit(f1,"QR")
